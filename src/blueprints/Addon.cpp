...
{
    const sc2::Unit* assignee = GetValidAssignee(order_);
...
// ----------------------------------------------------------------------------
const sc2::Unit* Addon::GetValidAssignee(const Order* order_)
{
    // We return this pointer;
    const sc2::Unit* assignee = nullptr;
    ...
    if(order_->assignee == sc2::NullTag)
    {
        ...
        Units parentBuildings = gAPI->observer().GetUnits(
